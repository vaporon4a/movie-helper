...
func (f *fakeTransport) OpenPoll(_ context.Context, _ int64, labels []string, _ time.Time) (string, int, error) {
	f.opened++
	if f.openErr != nil {
		return "", 0, f.openErr
	}
...
func (f *fakeTransport) SendSummary(context.Context, int64, string, int64, bool) (int, error) {
	f.summaries++
	return 201, nil
}

func (f *fakeTransport) SendMovies(_ context.Context, _ int64, movies []movieclub.Recommendation, _ movieclub.Catalog) ([]int, error) {
	f.pages = append(f.pages, append([]movieclub.Recommendation(nil), movies...))
	return []int{202}, nil
}

func TestRunnerPersistsPollSelectionAndSecondPage(t *testing.T) {
	...
	runner := &movieclub.Runner{Store: store, Telegram: transport, Catalog: fakeCatalog{}, Allowed: map[int64]bool{chat: true}, Log: slog.New(slog.NewTextHandler(io.Discard, nil)), Now: func() time.Time { return now }}
	roundID, err := runner.Start(ctx, 2, chat, 10*time.Minute)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = runner.More(ctx, -200, roundID); err == nil {
		t.Fatal("another chat claimed second page")
	}
	if err = runner.More(ctx, chat, roundID); err != nil {
		t.Fatal(err)
	}
...
func TestUnknownPollDeliveryRequiresResolution(t *testing.T) {
	...
	runner := &movieclub.Runner{Store: store, Telegram: transport, Catalog: fakeCatalog{}, Allowed: map[int64]bool{chat: true}, Log: slog.New(slog.NewTextHandler(io.Discard, nil)), Now: func() time.Time { return now }}
	roundID, err := runner.Start(ctx, 10, chat, 10*time.Minute)
	if err != nil {
		t.Fatal(err)
	}
	if err = runner.Tick(ctx); err != nil {
	...
	if err = runner.Tick(ctx); err != nil || transport.opened != 1 {
