...
func TestConfiguration(t *testing.T) {
	...
	c, err := Parse(get)
	if err != nil || len(c.Chats) != 2 || len(c.FactWikiTitles) != 20 || len(c.Subreddits) != 3 || c.Subreddits[1] != "Pikabu" || c.GeminiDailyLimit != 20 || c.GeminiKey != "" || c.GeminiModel != "gemini-3.8-flash" || c.GroqModel != "qwen/qwen3.8-27b" || c.GroqDailyLimit != 20 || c.GroqKey != "" || c.TMDBToken != "" {
	...
	base["TMDB_API_TOKEN"] = "tmdb-secret"
	base["FACT_WIKI_TITLES"] = "Alien (film)|Alien (film)|The Matrix"
	c, err = Parse(get)
	if err != nil || c.GeminiDailyLimit != 0 || len(c.FactWikiTitles) != 2 || c.TMDBToken != "tmdb-secret" {
		t.Fatal(c, err)
	}
}
